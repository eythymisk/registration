...
struct rigidtform {
    ...
    /* Constructor from a vector of parameters */
    rigidtform(const Eigen::VectorXf& parameters)
        : R(Rot::eul2rotm(parameters.tail<3>())),  // Initialize R using the last 3 elements for Euler angles
          t(parameters.head<3>()) {                 // Initialize t using the first 3 elements for translation
        composeAugmentedMatrix();                   // Call the method to compose the augmented matrix
    }
